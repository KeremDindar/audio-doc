...
class TranscriptionViewController: UIViewController {
    ...
    // MARK: - Properties
    internal var audioURL: URL
    internal var recordingDate: Date
    internal var recordingDuration: TimeInterval
    internal var selectedImage: UIImage?
    internal var transcription: String = ""
    internal var summaryKeywords: [String] = []
    internal var tags: [Tag] = []
    internal let maxTagCount = 3
    internal var recording: Recording?  
    
    // Manager instances
    internal lazy var audioPlayerManager: AudioPlayerManager = {
    ...
    internal lazy var summaryLabel: UILabel = {
        let label = UILabel()
    ...
    internal lazy var keywordsLabel: UILabel = {
        let label = UILabel()
        label.text = "Summary keywords loading..."
        label.textAlignment = .left
    ...
    internal lazy var transcriptionTextView: UITextView = {
    ...
    internal lazy var audioControlsView: UIView = {
        let view = UIView()
    ...
    internal lazy var totalTimeLabel: UILabel = {
        let label = UILabel()
        label.text = "00:00"
        label.font = .systemFont(ofSize: 12)
        label.textColor = .darkGray
    ...
    internal lazy var backwardButton: UIButton = {
    ...
    internal lazy var speedButton: UIButton = {
        let button = UIButton(type: .system)
    ...
    internal lazy var galleryButton: UIButton = {
        let button = UIButton(type: .system)
    ...
    internal lazy var deleteImageButton: UIButton = {
        let button = UIButton(type: .system)
    ...
    internal lazy var tagsScrollView: UIScrollView = {
    ...
    internal lazy var tagsStackView: UIStackView = {
    ...
    internal lazy var tagInputContainerView: UIView = {
        let view = UIView()
    ...
    internal lazy var tagInputView: UIView = {
        let view = UIView()
    ...
    internal lazy var tagInputTextField: UITextField = {
    ...
    internal lazy var tagInputAddButton: UIButton = {
        let button = UIButton(type: .system)
    ...
    // Add progress slider
    internal lazy var progressSlider: UISlider = {
        let slider = UISlider()
    ...
    // Add loading view
    internal lazy var loadingContainer: UIView = {
        let view = UIView()
    ...
    override func viewDidLoad() {
        ...
        // Calculate estimated duration for totalTimeLabel
        totalTimeLabel.text = audioPlayerManager.formatTime(recordingDuration)
        
        // Set title based on whether we're loading an existing recording or creating a new one
        if let recording = recording {
            // Use the title from existing recording
            titleLabel.text = recording.title
            
            // Only disable gallery button and make it look disabled
            galleryButton.isEnabled = false
            galleryButton.tintColor = .systemGray3
        } else {
            // For new recordings, get the next number from UserDefaults
        ...
        // Add tap gesture to dismiss tag input
        ...
        // Initialize transcriptionTextView
        ...
        // Load existing recording data if available
        if let recording = recording {
    ...
    deinit {
        // Controller yok edildiğinde tüm notification dinleyicileri kaldır
        NotificationCenter.default.removeObserver(self)
        
        // Audio player'ı temizle
        audioPlayerManager.cleanup()
    }
    ...
    private func loadExistingRecordingData(_ recording: Recording) {
        ...
        if let imageURLString = recording.imageURL {
            ...
            // Hide delete button for existing recordings
    ...
    private func startTranscription() {
        // Show loading message without blocking UI
        transcriptionTextView.text = "Transcribing audio..."
        
        // Use TranscriptionManager to transcribe audio
        transcriptionManager.startTranscription { [weak self] transcription, error in
            guard let self = self else { return }
            
            DispatchQueue.main.async {
                if let error = error {
                    self.transcriptionTextView.text = "Transcription failed. Error: \(error.localizedDescription)"
                ...
                if let transcription = transcription {
                    ...
                    // Generate keywords only when transcription is complete
                    if !transcription.isEmpty {
                        self.transcriptionManager.generateKeywords(from: transcription) { keywords in
                            DispatchQueue.main.async {
    ...
    internal func updatePlayerUI() {
        // Use currentTime and duration from audioPlayerManager
    ...
    @objc internal func sliderValueChanged() {
        // Update current time label while sliding
    ...
    @objc internal func tagInputTextFieldTapped(_ sender: UITextField) {
        // Kullanıcı text field'a tıkladığında klavyeyi aç
        sender.becomeFirstResponder()
    }
    
    @objc internal func handleTapOutsideTagInput(_ sender: UITapGestureRecognizer) {
        // Input view dışına tıklandığında kontrol et
    ...
    @objc internal func addTagButtonTapped() {
        ...
        // Hide the tag input overlay
        hideTagInput()
        
        // Create a tag with random color
        let tag = Tag(text: tagText, color: Tag.randomColor())
        addTag(tag)
    }
    ...
    @objc internal func keyboardWillShow(notification: NSNotification) {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            // Klavyenin yarısı kadar yukarı kaydır
    ...
    @objc internal func keyboardWillHide(notification: NSNotification) {
        // Input view'ı orijinal pozisyonuna geri getir
        UIView.animate(withDuration: 0.3) {
    ...
    private func tagRecording() {
        // Only allow adding tags if we're under the limit
        ...
        // Önce overlay'i göster, animasyonu tamamla
        UIView.animate(withDuration: 0.3, animations: {
            self.tagInputContainerView.alpha = 1
        }, completion: { _ in
            // Notification'ları dinlemeye başla
    ...
    private func createAndSaveRecording(audioURL: String, imageURL: String?) {
        ...
        // Get the next voice number from UserDefaults
        let defaults = UserDefaults.standard
        let nextVoiceNumber = defaults.integer(forKey: "nextVoiceNumber")
        
        // Create title with format "Voice 0001", "Voice 0002", etc.
        let formattedNumber = String(format: "%04d", nextVoiceNumber > 0 ? nextVoiceNumber : 1)
